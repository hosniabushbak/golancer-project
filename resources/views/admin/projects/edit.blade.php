...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.projects.update", [$project->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="btnlink">{{ trans('cruds.project.fields.btnlink') }}</label>
                <input class="form-control {{ $errors->has('btnlink') ? 'is-invalid' : '' }}" type="text" name="btnlink" id="btnlink" value="{{ old('btnlink', $project->btnlink) }}" required>
                @if($errors->has('btnlink'))
                    <div class="invalid-feedback">
                        {{ $errors->first('btnlink') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.project.fields.btnlink_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required" for="btn_id">{{ trans('cruds.project.fields.btn') }}</label>
                <select class="form-control select2 {{ $errors->has('btn') ? 'is-invalid' : '' }}" name="btn_id" id="btn_id" required>
                    @foreach($btns as $id => $entry)
                        <option value="{{ $id }}" {{ (old('btn_id') ? old('btn_id') : $project->btn->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('btn'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.project.fields.btn_helper') }}</span>
            </div>
            <div class="form-group">
