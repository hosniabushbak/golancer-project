...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sliders.update", [$slider->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="image_2">{{ trans('cruds.slider.fields.image_2') }}</label>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.slider.fields.image_2_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label class="required" for="btn_link">{{ trans('cruds.slider.fields.btn_link') }}</label>
                <input class="form-control {{ $errors->has('btn_link') ? 'is-invalid' : '' }}" type="text" name="btn_link" id="btn_link" value="{{ old('btn_link', $slider->btn_link) }}" required>
                @if($errors->has('btn_link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('btn_link') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.slider.fields.btn_link_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    init: function () {
@if(isset($slider) && $slider->image_2)
      var file = {!! json_encode($slider->image_2) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, file.preview ?? file.preview_url)
      file.previewElement.classList.add('dz-complete')
