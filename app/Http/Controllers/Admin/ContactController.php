...
class ContactController extends Controller
{
    ...
    public function store(StoreContactRequest $request)
    {
        ...
        if ($request->input('banner', false)) {
            $contact->addMedia(storage_path('tmp/uploads/' . basename($request->input('banner'))))->toMediaCollection('banner');
        }
    ...
    public function update(UpdateContactRequest $request, Contact $contact)
    {
        ...
        if ($request->input('banner', false)) {
            if (! $contact->banner || $request->input('banner') !== $contact->banner->file_name) {
                if ($contact->banner) {
                    $contact->banner->delete();
                }
                $contact->addMedia(storage_path('tmp/uploads/' . basename($request->input('banner'))))->toMediaCollection('banner');
            }
        } elseif ($contact->banner) {
            $contact->banner->delete();
        }
