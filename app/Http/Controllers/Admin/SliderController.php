...
class SliderController extends Controller
{
    ...
    public function store(StoreSliderRequest $request)
    {
        ...
        if ($request->input('image_2', false)) {
            $slider->addMedia(storage_path('tmp/uploads/' . basename($request->input('image_2'))))->toMediaCollection('image_2');
        }
    ...
    public function update(UpdateSliderRequest $request, Slider $slider)
    {
        ...
        if ($request->input('image_2', false)) {
            if (! $slider->image_2 || $request->input('image_2') !== $slider->image_2->file_name) {
                if ($slider->image_2) {
                    $slider->image_2->delete();
                }
                $slider->addMedia(storage_path('tmp/uploads/' . basename($request->input('image_2'))))->toMediaCollection('image_2');
            }
        } elseif ($slider->image_2) {
            $slider->image_2->delete();
        }
